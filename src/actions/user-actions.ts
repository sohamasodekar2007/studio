...
/**
 * INTERNAL HELPER: Reads the users.json file, performs initialization,
...
async function readAndInitializeUsersInternal(): Promise<UserProfile[]> {
    ...
    for (const user of users) {
        ...
        if (currentUser.role === undefined || (currentUser.role !== 'Admin' && currentUser.role !== 'User') || currentUser.role !== derivedRole) {
    ...
    const adminUserIndex = processedUsers.findIndex(u => u.email?.toLowerCase() === primaryAdminEffectiveEmail);
