...
.refine(data => {
    ...
    const hasAnyWeightage = (data.stream === 'PCM' && (data.weightagePhysics !== undefined || data.weightageChemistry !== undefined || data.weightageMaths !== undefined)) ||
                           (data.stream === 'PCB' && (data.weightagePhysics !== undefined || data.weightageChemistry !== undefined || data.weightageBiology !== undefined));
...
export default function CreateTestPage() {
  ...
  const [lessons, setLessonsState] = useState<string[]>([]); 
  ...
  const selectedSubjectWatch = form.watch('subject' as any); 
 ...
 useEffect(() => {
    ...
    const commonValuesToPreserve: Partial<TestCreationFormValues> = {
    ...
    if (selectedTestType === 'full_length') {
      form.reset({
        ...
        selectedQuestionIds: undefined as any, 
  ...
  useEffect(() => {
    ...
    if (currentValues.testType === 'chapterwise' && currentValues.subject) {
        ...
        setLessonsState([]); 
        form.setValue('lesson' as any, ''); 
        form.setValue('selectedQuestionIds' as any, []); 
        setAvailableQuestions([]); 
        getLessonsForSubject(currentValues.subject)
            .then(setLessonsState)
  ...
  useEffect(() => {
     if (selectedTestType === 'chapterwise') {
         const currentValues = form.getValues();
         const selectedIds = (currentValues as Extract<TestCreationFormValues, { testType: 'chapterwise' }>).selectedQuestionIds || [];
  ...
  const handleAutoSelect = useCallback(() => {
    ...
    if (availableQuestions.length > 0 && countNum > 0) {
        ...
        form.setValue('selectedQuestionIds' as any, selectedIds); 
  ...
  const onSubmit = async (data: TestCreationFormValues): Promise<void> => {
    ...
    const baseData: Pick<GeneratedTest, 'test_code' | 'name' | 'duration' | 'type' | 'audience' | 'createdAt'> = {
        test_code: testCode,
        name: data.name,
    ...
    try {
        if (data.testType === 'chapterwise') {
            ...
            const selectedIds = chapterwiseData.selectedQuestionIds || []; 
            const selectedQsData = availableQuestions.filter(q => selectedIds.includes(q.id));
            
            if (selectedQsData.length === 0) {
                throw new Error("No questions selected for chapterwise test. Please select questions.");
            }
             if (selectedQsData.length !== chapterwiseData.count) {
                toast({
                    variant: "destructive",
                    title: "Question Count Mismatch",
                    description: `You selected ${selectedQsData.length} questions, but specified ${chapterwiseData.count}. Please adjust your selection or count.`,
                });
                setIsLoading(false);
                return;
            }
            ...
            testDefinition = {
             ...
             if (totalAvailableBankQuestions < fullLengthData.totalQuestions) {
                 throw new Error(`Not enough questions in bank (${totalAvailableBankQuestions}) for requested total (${fullLengthData.totalQuestions}). Try broader filters or add more questions.`);
             }

             const totalQuestionsNeeded = fullLengthData.totalQuestions;
             let weightages: Record<string, number | undefined> = {
                Physics: fullLengthData.weightagePhysics,
                Chemistry: fullLengthData.weightageChemistry,
                Maths: fullLengthData.stream === 'PCM' ? fullLengthData.weightageMaths : undefined,
             ...
             if (fullLengthData.stream === 'PCM' && weightages.Physics === undefined && weightages.Chemistry === undefined && weightages.Maths === undefined) {
                 ...
                 weightages = { Physics: 33, Chemistry: 33, Biology: 34 };
             }
            ...
            subjectsInTest.forEach(subject => {
                targetCounts[subject] = Math.max(0, Math.round(((weightages[subject] ?? 0) / 100) * totalQuestionsNeeded));
            ...
            while (diff !== 0 && attempts < 100) { 
                const adjustableSubjects = subjectsInTest.filter(s => (diff > 0 && (allSubjectQuestions[s]?.length || 0) > targetCounts[s]) || (diff < 0 && targetCounts[s] > 0 && (weightages[s] ?? 0) > 0) );
            ...
            calculatedSum = Object.values(targetCounts).reduce((s, c) => s + c, 0);
            diff = totalQuestionsNeeded - calculatedSum;
            if (diff !== 0) {
                const primarySubject = subjectsInTest.sort((a,b) => (weightages[b] ?? 0) - (weightages[a] ?? 0))[0];
                targetCounts[primarySubject] += diff;
            }
            ...
            for (const subject of subjectsInTest) {
                ...
                if (countForSubject <= 0) continue;

                const availableQs = shuffleArray([...allSubjectQuestions[subject] || []]);
                if (availableQs.length < countForSubject) {
                     console.warn(`Not enough questions for ${subject} (needed ${countForSubject}, found ${availableQs.length}). Using all available.`);
                }
            ...
            testDefinition = {
                ...
                weightage: { 
                    physics: weightages.Physics ?? 0,
                    chemistry: weightages.Chemistry ?? 0,
                    maths: fullLengthData.stream === 'PCM' ? (weightages.Maths ?? 0) : undefined,
                    biology: fullLengthData.stream === 'PCB' ? (weightages.Biology ?? 0) : undefined,
                },
        ...
        if (testDefinition) {
            ...
            form.reset( { 
                testType: 'chapterwise', 
                name: '', duration: 60, type: 'FREE', audience: 'Dropper',
                subject: '', lesson: '', examFilter: 'all', selectedQuestionIds: [], count: 20,
                stream: 'PCM', totalQuestions: 50, weightagePhysics: 33, weightageChemistry: 33, weightageMaths: 34, weightageBiology: undefined,
            });
            setAvailableQuestions([]); 
            setSelectedQuestionObjects([]);
        } else {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <FormItem className="space-y-3">
                            <FormControl>
                                <RadioGroup
                                    ...
                                    value={field.value as 'chapterwise' | 'full_length'} 
                                    className="flex flex-col space-y-2 md:flex-row md:space-y-0 md:space-x-4"
                                    disabled={isLoading}
                                >
                                    <FormItem className="flex items-center space-x-3 space-y-0">
                                        <FormControl><RadioGroupItem value="chapterwise" id="r1" /></FormControl>
                                        <Label htmlFor="r1" className="font-normal">Chapterwise</Label>
                                    </FormItem>
                                    <FormItem className="flex items-center space-x-3 space-y-0">
                                        <FormControl><RadioGroupItem value="full_length" id="r2" /></FormControl>
                                        <Label htmlFor="r2" className="font-normal">Full Length</Label>
                                    </FormItem>
                                </RadioGroup>
                            </FormControl>
                            <FormMessage />
            ...
            {selectedTestType === 'chapterwise' && (
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <FormField control={form.control} name="subject" render={({ field }) => (<FormItem><FormLabel>Subject *</FormLabel><Select onValueChange={field.onChange} value={field.value as string ?? ''} disabled={isLoading || isLoadingSubjects}><FormControl><SelectTrigger><SelectValue placeholder={isLoadingSubjects ? "Loading..." : "Select Subject"} /></SelectTrigger></FormControl><SelectContent>{subjects.map((sub) => <SelectItem key={sub} value={sub}>{sub}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
                        <FormField control={form.control} name="lesson" render={({ field }) => (
                            <FormItem className="flex flex-col">
                                <FormLabel>Lesson *</FormLabel>
                                <Popover open={lessonPopoverOpen} onOpenChange={setLessonPopoverOpen}><PopoverTrigger asChild><FormControl><Button variant="outline" role="combobox" className={cn("w-full justify-between", !field.value && "text-muted-foreground")} disabled={isLoading || isLoadingLessons || !selectedSubjectWatch}><BookOpen className="mr-2 h-4 w-4 opacity-50" />{(field.value as string) ? (field.value as string) : (isLoadingLessons ? "Loading..." : "Select Lesson")}<ChevronsUpDown className="ml-auto h-4 w-4 shrink-0 opacity-50" /></Button></FormControl></PopoverTrigger>
                        ...
                        <FormField control={form.control} name="selectedQuestionIds" render={({ field }) => (
                            <FormItem>
                                <div className="mb-2 flex justify-between items-center">
                                    <FormLabel>Select Questions * ({(form.watch('selectedQuestionIds' as any, []) as string[]).length} / {totalAvailableCount})</FormLabel>
                                    {availableQuestions.length > 0 && (
                                        <div className="space-x-2">
                                          <Button type="button" size="sm" variant="outline" onClick={handleAutoSelect} disabled={isLoading || isLoadingQuestions || !chapterwiseCountWatch || chapterwiseCountWatch === 0}>Auto-Pick {chapterwiseCountWatch}</Button>
                                          <Button type="button" size="sm" variant="outline" onClick={() => field.onChange(availableQuestions.map(q => q.id))} disabled={isLoading || isLoadingQuestions}>Select All ({totalAvailableCount})</Button>
                                          <Button type="button" size="sm" variant="outline" onClick={() => field.onChange([])} disabled={isLoading || isLoadingQuestions}>Deselect All</Button>
                                        </div>
                                    )}
                                </div>
                                <FormControl>
                                    <ScrollArea className="h-72 w-full rounded-md border p-4 bg-muted/20">
                                        {isLoadingQuestions ? (<p className="text-center text-muted-foreground p-4"><Loader2 className="h-5 w-5 animate-spin inline mr-2"/>Loading questions...</p>) : 
                                         availableQuestions.length === 0 ? (<p className="text-center text-muted-foreground p-4">No questions found. Adjust filters or add questions to the bank.</p>) : 
                                         (<div className="space-y-2">
                                            {availableQuestions.map((item) => (
                                                <FormField key={item.id} control={form.control} name="selectedQuestionIds" render={({ field: checkboxField }) => (
                                                    <FormItem key={item.id} className="flex flex-row items-start space-x-3 space-y-0 p-3 border rounded bg-background hover:bg-accent/10 transition-colors">
                                                        <FormControl>
                                                            <Checkbox
                                                                checked={(checkboxField.value as string[])?.includes(item.id)}
                                                                onCheckedChange={(checked) => 
                                                                    checked ? checkboxField.onChange([...((checkboxField.value as string[]) || []), item.id]) 
                                                                            : checkboxField.onChange((checkboxField.value as string[])?.filter(id => id !== item.id))
                                                                }
                                                            />
                                                        </FormControl>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t">
                            ...
                            <FormField control={form.control} name="weightagePhysics" render={({ field }) => (<FormItem><FormLabel>Physics</FormLabel><FormControl><Input type="number" min="0" max="100" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} disabled={isLoading} /></FormControl><FormMessage /></FormItem>)} />
                            <FormField control={form.control} name="weightageChemistry" render={({ field }) => (<FormItem><FormLabel>Chemistry</FormLabel><FormControl><Input type="number" min="0" max="100" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} disabled={isLoading} /></FormControl><FormMessage /></FormItem>)} />
                            {fullLengthStreamWatch === 'PCM' && (
                                <FormField control={form.control} name="weightageMaths" render={({ field }) => (<FormItem><FormLabel>Maths</FormLabel><FormControl><Input type="number" min="0" max="100" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} disabled={isLoading} /></FormControl><FormMessage /></FormItem>)} />
                            )}
                            {fullLengthStreamWatch === 'PCB' && (
                                <FormField control={form.control} name="weightageBiology" render={({ field }) => (<FormItem><FormLabel>Biology</FormLabel><FormControl><Input type="number" min="0" max="100" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} disabled={isLoading} /></FormControl><FormMessage /></FormItem>)} />
                            )}
                        </div>
                         <Alert variant="default" className="bg-blue-50 border-blue-200 text-blue-800 dark:bg-blue-900/20 dark:border-blue-700/50 dark:text-blue-300">
                            ...
                            <AlertDescription>Questions are auto-selected based on criteria. Ensure enough questions exist in the bank for the chosen filters and weightages. If weightages are left blank for a stream, they will default (e.g., Physics 33%, Chemistry 33%, Maths/Bio 34%).</AlertDescription>
                        </Alert>
                    </CardContent>
                </Card>
            )}
            ...
            <div className="flex justify-end">
                <Button type="submit" disabled={isLoading || isSubmittingQuestions}>
                    {(isLoading || isSubmittingQuestions) ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <PlusCircle className="mr-2 h-4 w-4" />}
                    Create Test
                </Button>
            </div>
          </form>
