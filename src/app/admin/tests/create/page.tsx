...
const ChapterwiseSchemaInternal = commonTestFieldsSchema.extend({
...
const FullLengthSchemaInternal = commonTestFieldsSchema.extend({
...
// Ensure the schemas are correctly defined before use in discriminatedUnion
const ChapterwiseSchema = ChapterwiseSchemaInternal;
const FullLengthSchema = FullLengthSchemaInternal;
...
export default function CreateTestPage() {
  ...
  useEffect(() => {
    const currentSubject = (form.getValues() as Extract<TestCreationFormValues, { testType: 'chapterwise' }>).subject;
    if (selectedTestType === 'chapterwise' && currentSubject) {
        setIsLoadingLessons(true);
        setLessons([]);
        form.setValue('lesson' as any, ''); 
        form.setValue('selectedQuestionIds' as any, []); 
        setAvailableQuestions([]);
        getLessonsForSubject(currentSubject)
            .then(setLessons)
            .catch(err => toast({ variant: "destructive", title: "Error", description: `Could not load lessons for ${currentSubject}.` }))
    ...
    try {
        if (data.testType === 'chapterwise') {
            ...
            subjectsInTest.forEach(subject => {
                targetCounts[subject] = Math.max(0, Math.round((weightages[subject] / 100) * totalQuestionsNeeded));
             ...
             while (diff !== 0 && attempts < 100) {
                const adjustableSubjects = subjectsInTest.filter(s => (diff > 0) || (targetCounts[s] > 0 && weightages[s] > 0));
             ...
             if (diff !== 0) {
                 const primarySubject = subjectsInTest.sort((a, b) => weightages[b] - weightages[a])[0];
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <FormField control={form.control} name="selectedQuestionIds" render={({ field }) => (
                            <FormItem>
                                <div className="mb-2 flex justify-between items-center"><FormLabel>Select Questions * ({(form.watch('selectedQuestionIds' as any, []) as string[]).length} / {availableQuestions.length})</FormLabel>
