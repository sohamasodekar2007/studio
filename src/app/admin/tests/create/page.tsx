...
// Base schema for common fields (testName, duration, accessType, audience)
...
// Schema for Chapterwise Test specific details
...
// Combined Chapterwise Schema with discriminator
const ChapterwiseSchema = BaseTestSchema.merge(ChapterwiseDetailsSchema).extend({
  testType: z.literal('chapterwise'),
});

// Schema for individual subject configuration within a Full-Length Test
const SubjectConfigSchema = z.object({
  subjectName: z.string(),
  lessons: z.array(z.object({
    lessonName: z.string(),
    weightage: z.number().min(0).max(100).default(0), // Default to 0 if not specified
    questionCount: z.number().min(0).default(0),     // Default to 0 if not specified
  })).default([]),
  totalSubjectWeightage: z.number().min(0).max(100).default(0),
  totalSubjectQuestions: z.number().min(0).default(0),
});

// Schema for Full-Length Test specific details
...
// Combined Full-Length Schema with discriminator and refinements
const FullLengthSchema = BaseTestSchema.merge(FullLengthDetailsSchema).extend({
  testType: z.literal('full_length'),
}).refine(data => {
  if (data.subjectsConfig.length > 0) {
    const totalWeightageSum = data.subjectsConfig.reduce((sum, subj) => sum + (subj.totalSubjectWeightage || 0), 0);
    return Math.abs(totalWeightageSum - 100) < 0.1; // Allow small floating point inaccuracies
  }
  return true; // If no subjectsConfig, this refinement passes
...
const examSubjectMap: Record<ExamOption, string[]> = {
    ...
    "CUET": ["Physics", "Chemistry", "Mathematics", "Biology"], // Example, can be customized
    "AIEEE": ["Physics", "Chemistry", "Mathematics"], // Legacy, but for completeness
    "Other": ["Physics", "Chemistry", "Mathematics", "Biology"], // Default for 'Other'
};
...
export default function CreateTestPage() {
  ...
  useEffect(() => {
    if (testType === 'full_length' && subjectsConfigValues && subjectsConfigValues.length > 0 && overallTotalQuestions > 0) {
        ...
        const newConfigs = subjectsConfigValues.map((config, subjectIndex) => {
            ...
            if (totalWeightageSum > 0 && config.totalSubjectWeightage > 0) { // Ensure weightage is positive
                 questionsForSubject = Math.round((config.totalSubjectWeightage / totalWeightageSum) * overallTotalQuestions);
            } else if (totalWeightageSum === 0 && subjectsConfigValues.length > 0) { // Distribute equally if no weightages
            ...
            if (config.lessons && config.lessons.length > 0 && questionsForSubject > 0) {
                ...
                } else { // If no lesson weightages, distribute equally
                ...
                // Adjust last lesson in subject if sum doesn't match due to rounding
        ...
        // Final adjustment for overallTotalQuestions across subjects
        if (distributedQuestionsSum !== overallTotalQuestions && newConfigs.length > 0) {
            ...
            // Re-distribute questions within the last subject's lessons if its totalSubjectQuestions changed
             const lastSubjectConfig = form.getValues(`subjectsConfig.${lastConfigIndex}`);
             if (lastSubjectConfig && lastSubjectConfig.lessons.length > 0) {
    ...
    try {
      if (data.testType === 'chapterwise') {
        ...
        testToSave = {
          ...
          lesson: data.lessons.length === 1 ? data.lessons[0] : data.lessons.join(', '), // Handle single/multiple lessons
        ...
        for (const subjectConfig of data.subjectsConfig) {
            for (const lessonConfig of subjectConfig.lessons) {
                if (lessonConfig.questionCount > 0) {
                    ...
                    // Note: English/Logical Reasoning for BITSAT are not explicitly handled for separate arrays here
                    // They will be part of allSelectedQuestionsForFLT if their subjectName matches & questions exist.
        ...
        if (allSelectedQuestionsForFLT.length !== data.overallTotalQuestions) {
            console.warn(`Actual questions selected (${allSelectedQuestionsForFLT.length}) for FLT does not match target (${data.overallTotalQuestions}). Check question availability and distribution logic.`);
            // Could add logic here to pad or trim if strictly needed, but this might lead to unbalanced tests
            // For now, use the actual number of questions selected.
        }
        ...
        testToSave = {
          ...
          maths_questions: mathsQs.length > 0 && (data.stream === 'PCM' || data.exam === 'BITSAT') ? mathsQs : undefined,
          biology_questions: biologyQs.length > 0 && data.stream === 'PCB' ? biologyQs : undefined,
          // BITSAT also has English & Logical Reasoning - these would need their own arrays if separated
          // For now, they'd be mixed into physics/chem/maths if `subjectName` matched, or just part of `allSelectedQuestionsForFLT`
  ...
  const renderQuestionPreviewItem = (q: QuestionBankItem) => {
    ...
    return (
        <span
            className="line-clamp-1 mathjax-content" // MathJax will process this
            title={text}
      ...
      <div className="space-y-6">
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                ...
                                <FormControl>
                                    <RadioGroup
                                        onValueChange={(value) => {
                                            ...
                                            // Reset form fields based on new test type
                                            form.reset({
                                                testType: newTestType,
                                                testName: form.getValues('testName'), // Keep common fields if desired
                    ...
                    {testType === 'full_length' && form.getValues('testType') === 'full_length' && (
                        <Card>
...
<changes>
  <change>
    <file>src/app/admin/tests/create/page.tsx</file>
    <description>Fix Zod schema definition for TestCreationSchema. The error "Cannot read properties of undefined (reading 'testType')" indicates that one of the schemas passed to `z.discriminatedUnion` is undefined. This version ensures the sub-schemas (ChapterwiseSchema, FullLengthSchema) are correctly defined with their `testType` literals before being used in the union. Also corrected a refinement condition and ensured `BaseTestSchema` doesn't prematurely include `testType` if it's meant to be the discriminator. The individual variant schemas now extend `BaseTestSchema` and add their specific `testType` literal.</description>
    <content><![CDATA[// src/app/admin/tests/create/page.tsx
'use client';
...
// Base schema for common fields (testName, duration, accessType, audience)
// IMPORTANT: Do NOT include 'testType' here if it's the discriminator
const BaseTestSchema = z.object({
  testName: z.string().min(3, "Test Name must be at least 3 characters."),
  duration: z.number().min(1, "Duration must be at least 1 minute.").positive("Duration must be positive."),
  accessType: z.enum(pricingTypes),
...
const FullLengthSchema = BaseTestSchema.merge(FullLengthDetailsSchema).extend({
  ...
  if (data.subjectsConfig && data.subjectsConfig.length > 0) { // Check if subjectsConfig exists and is not empty
    const totalWeightageSum = data.subjectsConfig.reduce((sum, subj) => sum + (subj.totalSubjectWeightage || 0), 0);
...
.refine(data => {
  if (data.subjectsConfig) { // Check if subjectsConfig exists
    for(const subject of data.subjectsConfig) {
      if (subject.lessons && subject.lessons.length > 0 && subject.lessons.some(l => (l.weightage || 0) > 0)) {
        const lessonWeightageSum = subject.lessons.reduce((sum, lesson) => sum + (lesson.weightage || 0), 0);
        if (Math.abs(lessonWeightageSum - 100) > 0.1) return false;
    ...
    if (testType === 'full_length' && subjectsConfigValues && subjectsConfigValues.length > 0 && overallTotalQuestions > 0) {
        ...
        if (distributedQuestionsSum !== overallTotalQuestions && newConfigs.length > 0) {
            ...
            form.setValue(`subjectsConfig.${lastConfigIndex}.totalSubjectQuestions`, Math.max(0, (currentTotal || 0) + diff));
             ...
             const lastSubjectConfig = form.getValues(`subjectsConfig.${lastConfigIndex}`);
             if (lastSubjectConfig && lastSubjectConfig.lessons && lastSubjectConfig.lessons.length > 0) { // Add null check for lessons
                const questionsForLastSubject = form.getValues(`subjectsConfig.${lastConfigIndex}.totalSubjectQuestions`) || 0;
                 const questionsPerLesson = Math.floor(questionsForLastSubject / lastSubjectConfig.lessons.length);
                 let remainder = questionsForLastSubject % lastSubjectConfig.lessons.length;
                 lastSubjectConfig.lessons.forEach((_, lessonIdx) => {
  ...
  const onSubmit = async (data: TestCreationFormValues) => {
    ...
    try {
      if (data.testType === 'chapterwise') {
        ...
        const fullLengthData = data as Extract<TestCreationFormValues, {testType: 'full_length'}>;
        let allSelectedQuestionsForFLT: TestQuestion[] = [];
        let physicsQs: TestQuestion[] = [];
        let chemistryQs: TestQuestion[] = [];
        let mathsQs: TestQuestion[] = [];
        ...
        if (fullLengthData.subjectsConfig) { // Ensure subjectsConfig is defined
            for (const subjectConfig of fullLengthData.subjectsConfig) {
                if (subjectConfig.lessons) { // Ensure lessons is defined
                    for (const lessonConfig of subjectConfig.lessons) {
                        if (lessonConfig.questionCount > 0) {
                            const questionsFromBank = await getQuestionsForLesson({ subject: subjectConfig.subjectName, lesson: lessonConfig.lessonName });
                            const shuffled = [...questionsFromBank].sort(() => 0.5 - Math.random());
        ...
        testToSave = {
          ...
          test_subject: fullLengthData.subjectsConfig?.map(s => s.subjectName) || [], // Handle undefined
          ...
          weightage: fullLengthData.subjectsConfig?.reduce((acc, curr) => { // Handle undefined
            if (curr.lessons) { // Handle undefined
                 acc[curr.subjectName] = curr.lessons.reduce((lessonAcc, lesson) => {
                    lessonAcc[lesson.lessonName] = lesson.weightage;
                    return lessonAcc;
                }, {} as Record<string, number>);
                                    ...
                                    <RadioGroup
                                        onValueChange={(value) => {
                                            ...
                                            form.reset(prev => ({
                                                ...prev,
                                                testType: newTestType,
                                                // Chapterwise specific reset
                                                subject: newTestType === 'chapterwise' ? '' : undefined,
                                                lessons: newTestType === 'chapterwise' ? [] : undefined,
                                                selectedQuestionIds: newTestType === 'chapterwise' ? [] : undefined,
                                                questionCount: newTestType === 'chapterwise' ? 20 : undefined,
                                                // Full-length specific reset
                                                exam: newTestType === 'full_length' ? 'MHT-CET' : undefined,
                                                stream: newTestType === 'full_length' ? undefined : undefined,
                                                overallTotalQuestions: newTestType === 'full_length' ? 50 : undefined,
                                                subjectsConfig: newTestType === 'full_length' ? [] : undefined,
