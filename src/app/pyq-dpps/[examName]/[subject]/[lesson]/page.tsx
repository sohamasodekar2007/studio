...
import type { QuestionBankItem, DifficultyLevel, UserDppLessonProgress, DppAttempt, Notebook } from '@/types'; // Reuse DPP types for now
...
export default function PyqDppPracticePage() {
  ...
  useEffect(() => {
    typesetMathJax();
  }, [currentQuestionIndex, allQuestions, showSolution, typesetMathJax]);
  ...
  useEffect(() => {
       if (user?.id && examName && subject && lesson) {
            ...
            // Fetch Notebooks (remains the same)
  ...
  const handleOptionSelect = (questionId: string, selectedOption: string) => {
   ...
   const checkAnswer = async () => {
       ...
       // Save PYQ attempt using the new action
       setIsSaving(true);
       try {
           ...
           setDppProgress(prev => {
               ...
               return {
                   ...(prev || { userId: user?.id || '', subject: subject || '', lesson: lesson || '', questionAttempts: {} }), // Base structure
   ...
   const goToNextQuestion = () => {
       if (currentQuestionIndex < filteredQuestions.length - 1) {
           ...
           router.push(`/pyq-dpps/${encodeURIComponent(examName)}`); // Navigate back to exam subject list
       }
   };

   // --- Rendering Functions (reused from DPP) ---
         ...
         return (
              <Card className="mt-6 bg-muted/40 dark:bg-muted/20 border-border">
                  ...
                  <CardContent>
                      ...
                      {hasImage && <div className="relative w-full max-w-lg h-64 mx-auto mt-4"><Image src={explanationImagePath!} alt={`Explanation Image`} layout="fill" objectFit="contain" className="rounded border" data-ai-hint="explanation image" onError={(e) => { console.error(`Error loading explanation image: ${explanationImagePath}`, e); }} unoptimized /></div>}
                  </CardContent>
              </Card>
         );
     };
   ...
   if (isLoading || authLoading) {
    ...
    if (!user) { router.push('/auth/login?redirect=/pyq-dpps'); return null; } // Redirect if not logged in
    ...
    if (filteredQuestions.length === 0) { return ( /* No Questions State */
         <div className="container mx-auto py-8 px-4 max-w-4xl space-y-6">
   ...
   return (
     <>
       ...
       {/* Add to Notebook Dialog */}
        {currentQuestion && user && (<AddToNotebookDialog isOpen={isNotebookModalOpen} onClose={handleCloseNotebookModal} notebooks={notebooks} onSave={handleSaveToNotebooks} isLoading={isSaving} userId={user.id} onNotebookCreated={(newNotebook) => setNotebooks(prev => [...prev, newNotebook])} />)}
     </>
   );
 }
