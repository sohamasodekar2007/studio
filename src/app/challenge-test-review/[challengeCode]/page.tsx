...
import { AlertTriangle, ArrowLeft, ArrowRight, CheckCircle, HelpCircle, Loader2, XCircle, Eye, Bookmark, Tag, FileText, ImageIcon } from 'lucide-react';
import type { Challenge, ChallengeParticipant, TestQuestion, UserAnswer, QuestionStatus, Notebook, BookmarkedQuestion } from '@/types';
...
export default function ChallengeTestReviewPage() {
  ...
  const viewingUserId = searchParams.get('userId'); // The user whose answers are being reviewed
  ...
  const fetchChallengeData = useCallback(async () => {
    ...
    try {
        ...
        if (!data.participants[viewingUserId] || data.participants[viewingUserId].status !== 'completed') {
            // If user didn't complete, or is not part of this challenge, redirect or show error
            setError("This user did not complete the challenge, or results are not available.");
            setChallengeData(null); // Clear any stale data
            return;
        }
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
        ...
        // Allow viewing if they are part of the challenge, even if not their own review (e.g. admin)
        // For now, restrict to only own review unless admin functionality is added later.
        console.warn("Attempting to view another user's challenge review. This might be restricted.");
        fetchChallengeData(); // Allow fetching, but UI might restrict if not own data.
      } else {
        fetchChallengeData();
      }
    }
  ...
  useEffect(() => {
    if (!isLoading && challengeData) typesetMathJax();
  }, [isLoading, challengeData, currentQuestionReviewIndex, typesetMathJax]);
  ...
  const currentReviewQuestion: TestQuestion | undefined = useMemo(() => allQuestionsFromChallenge?.[currentQuestionReviewIndex], [allQuestionsFromChallenge, currentQuestionReviewIndex]);
  const totalQuestions = useMemo(() => allQuestionsFromChallenge.length || 0, [allQuestionsFromChallenge, currentQuestionReviewIndex]);
  const optionKeys = useMemo(() => ["A", "B", "C", "D"], []);

  const userSubmittedAnswer = participantData?.answers?.find(ans => ans.questionId === currentReviewQuestion?.id);
  const userSelectedOptionKey = userSubmittedAnswer?.selectedOption;
  const correctOptionKey = currentReviewQuestion?.answer;
  const questionStatus = userSubmittedAnswer?.status || QuestionStatusEnum.NotVisited;
  ...
  const renderOptions = useCallback(() => { /* ... (same as chapterwise review) ... */ 
  ...
  const navigateReview = (direction: 'prev' | 'next') => { /* ... (same as chapterwise review) ... */ 
  ...
  const handleOpenNotebookModal = () => { /* ... (same as chapterwise review) ... */ 
  ...
  const handleSaveToNotebooks = async (selectedNotebookIds: string[], tags: string[]) => { /* ... (same as chapterwise review, ensure subject/lesson are from challengeData.testConfig) ... */ 
   ...
   const handleCreateNotebookCallback = useCallback(async (name: string) => { /* ... (same as chapterwise review) ... */ 
  ...
  if (!challengeData || !participantData || !currentReviewQuestion) { /* No data */ 
     return (
       <div className="container mx-auto py-8 px-4 max-w-3xl text-center">
        ...
        <Card className="shadow-lg border-border">
          <CardHeader className="p-4 sm:p-6">
            <div className="flex justify-between items-center flex-wrap gap-2">
              ...
              <Badge variant={QUESTION_STATUS_BADGE_VARIANTS[questionStatus]} className="capitalize">{questionStatus.replace('_', ' & ')}</Badge>
            </div>
          ...
          <CardContent className="px-4 sm:px-6">
            <div className="mb-5">{renderContent('question')}</div>
