  ...
  const onSubmit = async (data: EditUserFormValues) => {
    ...
    try {
        ...
        if (newEmail !== originalEmail) {
         ...
         const userRoleToMaintain = user.role || 'User'; 
         let finalModel = data.model;
         let finalExpiryDate = data.expiry_date;

         if (userRoleToMaintain === 'Admin') { 
             finalModel = 'combo';
       ...
       onUserUpdate({ ...result.user, role: userRoleToMaintain }); 
       onClose();
        ...
        <ScrollArea className="flex-grow pr-6 -mr-6"> {/* Add ScrollArea here */}
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
              ...
              <FormField control={form.control} name="phone" render={({ field }) => ( <FormItem><FormLabel>Phone Number</FormLabel><FormControl><Input type="tel" {...field} value={field.value || ""} disabled={true} /></FormControl><FormMessage /><p className="text-xs text-muted-foreground pt-1">Phone number cannot be edited here.</p></FormItem> )} />
